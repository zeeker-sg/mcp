...
async def test_locked_field_set(
    ...
    httpx_mock.add_response(
        url=_zeeker_schemas_url("zeeker-judgements"), json=_judgments_schema_payload()
    )
...
async def test_no_foreign_keys_or_indexes_leak(
    ...
    """T-02-schema-leak: no foreign_keys/indexes/triggers in response (extra='forbid')."""
    httpx_mock.add_response(
        url=_db_url("zeeker-judgements"), json=_judgments_db_payload(), is_reusable=True
    )
    httpx_mock.add_response(
...
async def test_row_count_null_passthrough(
    ...
    httpx_mock.add_response(
        url=_db_url("zeeker-judgements"), json=_judgments_db_payload(count=None), is_reusable=True
    )
    httpx_mock.add_response(
        url=_zeeker_schemas_url("zeeker-judgements"), json=_judgments_schema_payload()
    )
