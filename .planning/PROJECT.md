...
- ✓ Hidden-table and hidden-column enforcement — full validation sweep at the test layer — Phase 8 (`tests/test_hidden_data_enforcement.py` 29 parametrized cases via `core.config_lookup.hidden_columns_for` — D2-10 single-source-of-truth invariant honored)
- ✓ Test coverage for filter mapping, envelope shape, hidden-data enforcement, fragment joins, rate-limit windows, error mapping; gated live integration tests against `data.zeeker.sg` — Phase 8 (11/11 live tests passed 2026-05-17 via `UPSTREAM_URL=https://data.zeeker.sg ZEEKER_LIVE=1 uv run pytest -m live`; nightly + manual-dispatch workflow at `.github/workflows/live-tests.yml`)
- ✓ 5h30m soak under 50-concurrent synthetic load against `mcp.zeeker.sg` — Phase 8 (stability gates ✅: RSS max 102.7 MB / limit 256, 0 PoolTimeout, 0.031% error rate, no drift over 5.5h; raw latency exceeds the single-number PRD budget because it bundles cheap and fan-out-expensive tools — see Constraints update below)
...
| **PRD perf budget (p50<300ms / p95<1500ms) is per-tool-category, not aggregate** | Soak surfaced a bimodal latency distribution: cheap tools (75% of workload — list_databases / single-DB query_table / fetch) meet budget at low concurrency (p50 249ms / p95 463ms); expensive fan-out tools (search across 4 DBs, fragment 2-step cursor) are structurally above budget (~3.2s p50) because the cost is upstream-RTT × number-of-DBs. Adding more databases scales this further. | ✓ Phase 8 — verdict in `08-HUMAN-UAT.md` evidence block; pending todo in STATE.md to surface a client-side way to limit cross-DB fan-out before catalogue grows |
| **Stdlib-only soak harness** (no psutil / locust / hdrhistogram) | NFR-04 caps runtime deps at 6 packages; the harness adds zero. RSS sampled via `resource.getrusage` + `/proc/self/status`. | ✓ Phase 8 — `scripts/soak/*` + `tests/_corpus/soak_workload.py`; verified by `tests/test_dependency_footprint.py` |
| **`SOAK_BYPASS_TOKEN` production surface** (X-Soak-Bypass header skips rate limiter only; `/admin/metrics` returns 404 without it, not 401) | Soak driver needs to send sustained load against prod without tripping the rate limiter; the bypass is intentionally narrow (skips ONLY rate limiting; does not authorise any other surface). 404-on-absent (not 401/403) prevents confirming the surface to unauthorised callers. Constant-time compare on the token. | ✓ Phase 8 — `core/soak_auth.py` (commit `1eb74b4` rejects non-ASCII tokens); `core/middleware/rate_limit.py:143-145` scope; `core/admin.py:73` returns 404. Documented in `08-SOAK-BYPASS-REVIEW.md`. Operator teardown: unset env + restart container after each soak run. |
...
---
*Last updated: 2026-05-17 after Phase 8 (full-tests-24h-soak) — 6/6 plans complete, UAT complete (1 passed + 1 stability-passed-latency-breached with full evidence + decision basis in `08-HUMAN-UAT.md`), security audit SECURED (0/32 threats open), phase ticked complete in ROADMAP.md. Next: Phase 9 — submission PR to `anthropics/claude-for-legal`.*
